...
class HistoryVideos {
  ...
  static Future<dynamic> displayHistory() async {
    ...
    for (int i = 0; i < historyVideos.length; i++) {
      for (int j = 0; j < theAllVideosListFortheSelectionPage.length; j++) {
        if (historyVideos[i] == theAllVideosListFortheSelectionPage[j].id) {
          historyVideoNotifier.value
              .add(theAllVideosListFortheSelectionPage[j]);
          historyLists.add(theAllVideosListFortheSelectionPage[j].id);
        }
      }
    }
    return historyLists;
