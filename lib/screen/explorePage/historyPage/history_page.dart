...
class HistoryPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final provi = Provider.of<HistoryProvider>(context, listen: false);
    return Scaffold(
      backgroundColor: colorBlack,
      appBar: AppBar(
        backgroundColor: colorBlack,
      ...
      body: Consumer<HistoryProvider>(
        ...
        builder: (context, value, child) {
          if (value.historyVideosList.isEmpty) {
            return const Padding(
              padding: EdgeInsets.only(top: 10),
              child: Center(
                child: Text(
            ...
            final temp = value.historyVideosList.reversed.toList();
            return ListView.builder(
              itemBuilder: ((context, index) {
                return Padding(
                  padding: const EdgeInsets.only(left: 6, right: 6),
                  child: Card(
                    ...
                    child: ListTile(
                        onTap: () {
                          Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => VideoPLayerPage(
                                videoList: temp, initialIndex: index),
                          ));
                        },
                        leading: FutureBuilder<Uint8List?>(
                          future: temp[index].thumbnailData,
                          builder: (BuildContext context,
                              AsyncSnapshot<Uint8List?> snapshot) {
                            if (snapshot.connectionState ==
                                    ConnectionState.done &&
                        ...
                        subtitle: Text(
                          temp[index].relativePath.toString(),
                          maxLines: 1,
                          style: const TextStyle(
                              fontSize: 12, color: Colors.blueGrey),
                        ),
              ...
              itemCount: value.historyVideosList.length,
            );
          }
        },
      ),
